...
# This script takes as input an HTML file generated by Sphinx using autodoc
# commands and generates a new HTML file with just the documentation-related
# HTML (i.e., with the header, sidebar, footer, etc. removed).
#
# The result can then be incorporated into a Sphinx api_ref directory as
# "raw HTML".
#
# The reason behind this silliness is that Sphinx autodoc generates nice
# API reference HTML files on my laptop for PyImfit (because it can import
# the pyimfit package), but it *doesn't* work on readthedocs.org (because
# readthedocs.org prevents installation of Python extension modules, and
# so Sphinx can't import pyimfit and the autodoc generation fails.
#
# Intended use:
# On my laptop:
...
def ExtractUsefulText( input ):
    ...
    # extract the "section", then remove the H1 element
